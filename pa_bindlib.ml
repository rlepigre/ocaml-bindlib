...
let _ =
  ...
  | "letvar"; fv = expr LEVEL "simple"; id = LIDENT; "("; n = expr LEVEL "top";")"; 
	str = binding; fr = freshin; "in";  x = expr LEVEL "top" ->
  ...
  | "match"; e = SELF; "with"; "bind"; fv = expr LEVEL "simple";  
	...
	str = binding;  fr = freshin; "in"; g = LIDENT;
      "->"; f = expr LEVEL "top" -> 
	let e1 = 
	  <:expr<let $lid:g$ = Bindlib.msubst $lid:"#e"$ (Array.map Bindlib.free_of $lid:id$) in $f$>> 
	in
	let names = match str with
	  None -> <:expr<Bindlib.mbinder_names $lid:"#e"$>>
	| Some names -> 
