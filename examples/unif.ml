...
let abs s = box_apply (fun t -> Abs(s,t))
...
let new_uvar s = UVar (ref (Unknown s))
  ...
  let rec fn nargs t n = match t,n with
    | sh1, 0 ->
            ...
            gn (app h (new_avar s nargs) ) s' (n-1)
              ...
              in gn (box h) s2 arg2s
    | Arrow(s,s'), n when n > 0 ->
              abs s (bind (fVar s) "x" (fun x -> fn ((s,x)::nargs) s' (n-1)))
    | _ -> failwith "bug in imitate"
  ...
  | Abs _ as t ->
      ...
      let rec gn nv = function
          Abs (s,f) ->
            ...
            begin
	      let v = new_var (fVar s) name in
	      let t = subst f (free_of v) in
              print_string name;
              print_string " ";
              gn (nv + 1) t
            end
...
let idta = unbox (abs alpha (bind (fVar alpha) "x" (fun x -> x)))
...
let _ = test 10 (App(idta,c1)) (App(App(new_uvar(Arrow(alpha,(Arrow(Arrow(alpha,alpha),alpha)))),c1),idta))

let _ = let v = new_uvar(Arrow(alpha,alpha)) in
  test 10 (App(v,c1)) (App(f,App(v,c1)))

let _ = let v = new_uvar(Arrow(alpha,alpha)) in
  test 10 (App(v,App(v,c1))) (App(f,App(v,App(v,c1))))

let _ = let v = new_uvar(Arrow(alpha,Arrow(alpha,alpha))) in
